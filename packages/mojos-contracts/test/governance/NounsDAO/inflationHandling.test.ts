...
async function reset(): Promise<void> {
  ...
  const { address: timelockAddress } = await new mojosDaoExecutorFactory(deployer).deploy(
    govDelegatorAddress,
    timelockDelay,
  );
...
describe('mojosDAO#inflationHandling', () => {
  before(async () => {
  ...
  it('returns quorum votes and proposal threshold based on Mojos total supply', async () => {
    // Total Supply = 40
  ...
  it('rejects if proposing below threshold', async () => {
    ...
    ).revertedWith('mojosDAO::propose: proposer votes below proposal threshold');
  });

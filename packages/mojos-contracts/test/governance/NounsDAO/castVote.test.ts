...
import {
  ...
  mojosDaoLogicV1Harness,
  mojosDaoLogicV1Harness__factory as mojosDaoLogicV1HarnessFactory,
} from '../../../typechain';
...
async function deployGovernor(
  ...
  tokenAddress: string,
): Promise<mojosDaoLogicV1Harness> {
  const { address: govDelegateAddress } = await new mojosDaoLogicV1HarnessFactory(
    deployer,
  ).deploy();
  const params = [
    address(0),
  ...
  const { address: _govDelegatorAddress } = await (
    await ethers.getContractFactory('mojosDAOProxy', deployer)
  ).deploy(...params);

  return mojosDaoLogicV1HarnessFactory.connect(_govDelegatorAddress, deployer);
}
...
let gov: mojosDaoLogicV1Harness;
let targets: string[];
let values: string[];
...
async function reset() {
  ...
  token = await deploymojosToken(signers.deployer);

  await populateDescriptor(
    mojosDescriptorFactory.connect(await token.descriptor(), signers.deployer),
  );
  ...
  describe('We must revert if:', () => {
    ...
    it("There does not exist a proposal with matching proposal id where the current block number is between the proposal's start block (exclusive) and end block (inclusive)", async () => {
      await expect(gov.castVote(proposalId, 1)).revertedWith(
        'mojosDAO::castVoteInternal: voting is closed',
      );
    });
    ...
    it('Such proposal already has an entry in its voters set matching the sender', async () => {
      ...
      await expect(gov.connect(account0).castVote(proposalId, 1)).revertedWith(
        'mojosDAO::castVoteInternal: voter already voted',
      );
    });
  });

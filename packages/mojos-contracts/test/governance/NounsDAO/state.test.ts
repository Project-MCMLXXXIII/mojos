...
import {
  ...
  mojosDaoExecutorHarness,
  mojosDaoExecutorHarness__factory as mojosDaoExecutorHarnessFactory,
  mojosDaoImmutable,
  mojosDaoImmutable__factory as mojosDaoImmutableFactory,
} from '../../../typechain';
...
let gov: mojosDaoImmutable;
let timelock: mojosDaoExecutorHarness;
let delay: number;
...
async function makeProposal(
  ...
  timelock = await new mojosDaoExecutorHarnessFactory(deployer).deploy(deployer.address, delay);

  gov = await new mojosDaoImmutableFactory(deployer).deploy(
    timelock.address,
    token.address,
    address(0),
    deployer.address,
...
describe('mojosDAO#state/1', () => {
  ...
  it('Invalid for proposal not found', async () => {
    await makeProposal();
    await expect(gov.state(5)).revertedWith('mojosDAO::state: invalid proposal id');
  });

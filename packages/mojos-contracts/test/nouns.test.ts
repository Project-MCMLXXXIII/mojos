...
import { mojosDescriptor__factory as mojosDescriptorFactory, mojosToken } from '../typechain';
import { deploymojosToken, populateDescriptor } from './utils';
...
describe('mojosToken', () => {
  ...
  it('should allow the minter to mint a noun to itself and a reward noun to the noundersDAO', async () => {
    const receipt = await (await mojosToken.mint()).wait();

    const [, , , noundersNounCreated, , , , ownersNounCreated] = receipt.events || [];

    expect(await mojosToken.ownerOf(0)).to.eq(noundersDAO.address);
    ...
    expect(await mojosToken.ownerOf(1)).to.eq(deployer.address);
  ...
  it('should set symbol', async () => {
    expect(await mojosToken.symbol()).to.eq('NOUN');
  });

  it('should set name', async () => {
    expect(await mojosToken.name()).to.eq('mojos');
  });

  it('should allow minter to mint a noun to itself', async () => {
    await (await mojosToken.mint()).wait();

    const receipt = await (await mojosToken.mint()).wait();
    const nounCreated = receipt.events?.[3];

    expect(await mojosToken.ownerOf(2)).to.eq(deployer.address);
  ...
  it('should emit two transfer logs on mint', async () => {
    ...
    await (await mojosToken.setMinter(minter.address)).wait();
    await (await mojosToken.transferOwnership(creator.address)).wait();

    const tx = mojosToken.connect(minter).mint();

    await expect(tx)
      .to.emit(mojosToken, 'Transfer')
      .withArgs(constants.AddressZero, creator.address, 2);
    await expect(tx).to.emit(mojosToken, 'Transfer').withArgs(creator.address, minter.address, 2);
  });

  it('should allow minter to burn a noun', async () => {
    await (await mojosToken.mint()).wait();

    const tx = mojosToken.burn(0);
    await expect(tx).to.emit(mojosToken, 'NounBurned').withArgs(0);
  });

  it('should revert on non-minter mint', async () => {
    const account0AsNounErc721Account = mojosToken.connect(noundersDAO);
    await expect(account0AsNounErc721Account.mint()).to.be.reverted;
  });

  describe('contractURI', async () => {
    it('should return correct contractURI', async () => {
      expect(await mojosToken.contractURI()).to.eq(
        'ipfs://QmZi1n79FqWt2tTLwCqiy6nLM6xLGRsEPQ5JmReJQKNNzX',
      );
    });
    it('should allow owner to set contractURI', async () => {
      await mojosToken.setContractURIHash('ABC123');
      expect(await mojosToken.contractURI()).to.eq('ipfs://ABC123');
    });
    it('should not allow non owner to set contractURI', async () => {
      const [, nonOwner] = await ethers.getSigners();
      await expect(mojosToken.connect(nonOwner).setContractURIHash('BAD')).to.be.revertedWith(

...
import { deploymojosToken, deployWeth, populateDescriptor } from './utils';

chai.use(solidity);
const { expect } = chai;

describe('mojosAuctionHouse', () => {
  ...
  async function deploy(deployer?: SignerWithAddress) {
    ...
    return upgrades.deployProxy(auctionHouseFactory, [
      ...
      DURATION,
    ]) as Promise<mojosAuctionHouse>;
  }

  before(async () => {
    [deployer, noundersDAO, bidderA, bidderB] = await ethers.getSigners();

    mojosToken = await deploymojosToken(deployer, noundersDAO.address, deployer.address);
    weth = await deployWeth(deployer);
    mojosAuctionHouse = await deploy(deployer);

    const descriptor = await mojosToken.descriptor();

    await populateDescriptor(mojosDescriptorFactory.connect(descriptor, deployer));
  ...
  it('should revert if a second initialization is attempted', async () => {
    const tx = mojosAuctionHouse.initialize(
      mojosToken.address,
      weth.address,
      TIME_BUFFER,
      RESERVE_PRICE,
  ...
  it('should allow the noundersDAO to unpause the contract and create the first auction', async () => {
    const tx = await mojosAuctionHouse.unpause();
    await tx.wait();

    const auction = await mojosAuctionHouse.auction();
  ...
  it('should revert if a user creates a bid for an inactive auction', async () => {
    await (await mojosAuctionHouse.unpause()).wait();

    const { nounId } = await mojosAuctionHouse.auction();
    const tx = mojosAuctionHouse.connect(bidderA).createBid(nounId.add(1), {
      value: RESERVE_PRICE,
    });

    await expect(tx).to.be.revertedWith('Mojos not up for auction');
  });
  ...
  it('should revert if a user creates a bid for an expired auction', async () => {
    ...
    const { nounId } = await mojosAuctionHouse.auction();
    const tx = mojosAuctionHouse.connect(bidderA).createBid(nounId, {
      value: RESERVE_PRICE,
    });
  ...
  it('should revert if a user creates a bid less than the min bid increment percentage', async () => {
    ...
    const { nounId } = await mojosAuctionHouse.auction();
    await mojosAuctionHouse.connect(bidderA).createBid(nounId, {
      value: RESERVE_PRICE * 50,
    });
    const tx = mojosAuctionHouse.connect(bidderB).createBid(nounId, {
  ...
  it('should refund the previous bidder when the following user creates a bid', async () => {
    ...
    await mojosAuctionHouse.connect(bidderB).createBid(nounId, {
  ...
  it('should cap the maximum bid griefing cost at 30K gas + the cost to wrap and transfer WETH', async () => {
    ...
    const maliciousBid = await maliciousBidder
      .connect(bidderA)
      .bid(mojosAuctionHouse.address, nounId, {
        value: RESERVE_PRICE,
      });
    await maliciousBid.wait();

    const tx = await mojosAuctionHouse.connect(bidderB).createBid(nounId, {
      value: RESERVE_PRICE * 2,
      gasLimit: 1_000_000,
    });
  ...
  it('should emit an `AuctionBid` event on a successful bid', async () => {
    ...
    await expect(tx)
      .to.emit(mojosAuctionHouse, 'AuctionBid')
  ...
  it('should emit an `AuctionExtended` event if the auction end time is within the time buffer', async () => {
    ...
    const { nounId, endTime } = await mojosAuctionHouse.auction();
    ...
    await expect(tx)
      .to.emit(mojosAuctionHouse, 'AuctionExtended')
  ...
  it('should revert if auction settlement is attempted while the auction is still active', async () => {
    ...
    const tx = mojosAuctionHouse.connect(bidderA).settleCurrentAndCreateNewAuction();
  ...
  it('should emit `AuctionSettled` and `AuctionCreated` events if all conditions are met', async () => {
    ...
    await ethers.provider.send('evm_increaseTime', [60 * 60 * 25]); // Add 25 hours
    const tx = await mojosAuctionHouse.connect(bidderA).settleCurrentAndCreateNewAuction();
  ...
  it('should not create a new auction if the auction house is paused and unpaused while an auction is ongoing', async () => {
    ...
    await (await mojosAuctionHouse.pause()).wait();
  ...
  it('should create a new auction if the auction house is paused and unpaused after an auction is settled', async () => {
    ...
    const settleTx = mojosAuctionHouse.connect(bidderA).settleAuction();

    await expect(settleTx)
      .to.emit(mojosAuctionHouse, 'AuctionSettled')
      .withArgs(nounId, bidderA.address, RESERVE_PRICE);

    const unpauseTx = await mojosAuctionHouse.unpause();
  ...
  it('should settle the current auction and pause the contract if the minter is updated while the auction house is unpaused', async () => {
    ...
    await mojosToken.setMinter(constants.AddressZero);

    await ethers.provider.send('evm_increaseTime', [60 * 60 * 25]); // Add 25 hours

    const settleTx = mojosAuctionHouse.connect(bidderA).settleCurrentAndCreateNewAuction();

    await expect(settleTx)
      .to.emit(mojosAuctionHouse, 'AuctionSettled')
      .withArgs(nounId, bidderA.address, RESERVE_PRICE);

    const paused = await mojosAuctionHouse.paused();

    expect(paused).to.equal(true);
  });

  it('should burn a Mojos on auction settlement if no bids are received', async () => {
    await (await mojosAuctionHouse.unpause()).wait();

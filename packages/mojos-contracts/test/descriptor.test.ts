...
import { mojosDescriptor } from '../typechain';
import ImageData from '../files/image-data.json';
import { LongestPart } from './types';
import { deploymojosDescriptor, populateDescriptor } from './utils';
...
describe('mojosDescriptor', () => {
  let mojosDescriptor: mojosDescriptor;
  ...
  before(async () => {
    mojosDescriptor = await deploymojosDescriptor();
    ...
    await populateDescriptor(mojosDescriptor);
  });
  ...
  it('should generate valid token uri metadata when data uris are disabled', async () => {
    const BASE_URI = 'https://api.mojos.wtf/metadata/';

    await mojosDescriptor.setBaseURI(BASE_URI);
    await mojosDescriptor.toggleDataURIEnabled();

    const tokenUri = await mojosDescriptor.tokenURI(0, {
      background: 0,
  ...
  it('should generate valid token uri metadata when data uris are enabled', async () => {
    ...
    expect(name).to.equal('Mojos 0');
    expect(description).to.equal('Mojos 0 is a member of the mojos DAO');
  ...
  it.skip('should generate valid token uri metadata for all supported parts when data uris are enabled', async () => {
    ...
    for (let i = 0; i < max; i++) {
      const tokenUri = await mojosDescriptor.tokenURI(i, {
      ...
      expect(name).to.equal(`Mojos ${i}`);
      expect(description).to.equal(`Mojos ${i} is a member of the mojos DAO`);
      expect(image).to.not.be.undefined;

...
const fetchLayersAndOptionsURL = 'https://us-central1-mojosdao.cloudfunctions.net/fetchLayersAndOptionsWithSource'
const generateMojoWithOptions = 'https://us-central1-mojosdao.cloudfunctions.net/generateMojoUsingOptionsAndSourceAndBG'
const fetchSourcesURL = 'https://us-central1-mojosdao.cloudfunctions.net/fetchSources'
...
// hold mojos base64 data strings { base64: base64String, dominantColorHSL: [H,S,L]}
var mojosData = []
 ...
 * @param {DISPLAY_MODE} displayMode Mode to add mojo img with
 */
function addMojoImg(data, dominantColorHSL, displayMode) {

    var mojosDiv = document.getElementById("mojos")
    ...
    if (displayMode == DISPLAY_MODE.TILED) {
        ...
        mojosDiv.insertBefore(colThreeColumn, mojosDiv.children[0])
...
function displayModeChanged(element) {
    ...
    if (element.id == 'scaled-display-radio-btn') {
        ...
        mojosData.forEach(mojoData => {
            console.log(mojosData)
            addMojoImg(mojoData.base64, mojoData.dominantColorHSL, DISPLAY_MODE.SCALED)
        })
    } else {
        selectedDisplayMode = DISPLAY_MODE.TILED
        mojosData.forEach(mojoData => {
            addMojoImg(mojoData.base64, mojoData.dominantColorHSL, DISPLAY_MODE.TILED)
        })
    }
}

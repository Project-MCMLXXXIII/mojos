...
import classes from './MojoProfileVoteRow.module.css';
...
import { MojoVoteHistory } from '../ProfileActivityFeed';

interface MojoProfileVoteRowProps {
  proposal: Proposal;
  vote?: MojoVoteHistory;
}

const selectIconForMojoVoteActivityRow = (proposal: Proposal, vote?: MojoVoteHistory) => {
  if (!vote) {
...
const selectVotingInfoText = (proposal: Proposal, vote?: MojoVoteHistory) => {
  if (!vote) {
    if (proposal.status === ProposalState.PENDING || proposal.status === ProposalState.ACTIVE) {
      return 'Waiting for';
    }
...
const MojoProfileVoteRow: React.FC<MojoProfileVoteRowProps> = props => {
  ...
  return (
    <tr onClick={proposalOnClickHandler} className={classes.voteInfoRow}>
      <td className={classes.voteIcon}>{selectIconForMojoVoteActivityRow(proposal, vote)}</td>
...
export default MojoProfileVoteRow;
